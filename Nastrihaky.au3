...
Func Nastrihaky()
   ...
   For $i = 0 to $iPocetRadku - 2
	  ...
	  Send($aSvazky[$i])
	  Send("{ENTER}")
	  Local $error = WinWaitActive("Výbìr záhlaví technol. postupu","", 5)
	  If $error = 0 Then
   ...
   ;Zjištìní poètu aktuvních TP
	  Do
		 ...
		 If ControlGetText("Výbìr záhlaví technol. postupu","","Edit33") = 1 Then $iPocetAktTP = $iPocetAktTP + 1
		 Send("{DOWN}")
		 Sleep(100)
	  ...
	  Local $sCOP = ""
	  While $sCOP = ""
		 While ControlGetFocus("Výbìr záhlaví technol. postupu","") <> "Edit33"
			Send("{RIGHT}")
			Sleep(100)
		 WEnd
		 Local $sAkt = ControlGetText("Výbìr záhlaví technol. postupu","","Edit33")
		 If $sAkt = 1 Then
			Do
			   Send("{LEFT}")
			   Sleep(100)
			Until ControlGetFocus("Výbìr záhlaví technol. postupu","") = "Edit29"
			Local $sDatum = ControlGetText("Výbìr záhlaví technol. postupu","","Edit29")
	  ...
	  ErrorHandler($error)
   Next
   MsgBox($MB_ICONINFORMATION,"SUCCESS!!!","Všechny nástøiháky byly vytvoøeny!")
EndFunc
...
Func ProjetiTP($sOperace, $sNazev)
   ...
   Do
	  ...
	  Local $iPoziceBaru2 = DllStructGetData($tSCROLLBARINFO, "xyThumbTop")
   Until $iPoziceBaru1 = $iPoziceBaru2
   Local $sCOP = ControlGetText("Pøehled pracovních operací","","Edit20")
   ...
   While $sOperace <> $sCOP
	  ...
	  Send("{DOWN}")
	  Sleep(100)
	  $sCOP = ControlGetText("Pøehled pracovních operací","","Edit20")
	  $tSCROLLBARINFO = _GUIScrollBars_GetScrollBarInfoEx($hwndCtrl, $OBJID_CLIENT)
	  $iPoziceBaru2 = DllStructGetData($tSCROLLBARINFO, "xyThumbTop")
...
Func LogNastrihaku($sSvazek, $indexpole, $iPocetTP, $iPocetVodicu, $iCOP, $bChybejiciOP)
   ...
   If $iPocetTP > 1 Then
	  If $bChybejiciOP = True Then
		 FileWrite($hndl, $sSvazek & ";" & $iPocetVodicu & ";" & $iCOP & "; více akt. postupù" & "; nebyla nalezena hledaná op." & @CRLF)
	  Else
		 FileWrite($hndl, $sSvazek & ";" & $iPocetVodicu & ";" & $iCOP & "; více akt. postupù" & @CRLF)
	  EndIf
   ElseIf $iPocetTP = 1 Then
	  If $bChybejiciOP = True Then
		 FileWrite($hndl, $sSvazek & ";" & $iPocetVodicu & ";" & $iCOP & "; nebyla nalezena hledaná op." & @CRLF)
	  Else
...
Exit
   ...
   Exit
	  ...
	  Do
		 ...
		 Sleep(1000)
	  Until StringLen(ControlGetText("Nástøihový plán vodièù","","Edit21")) < 8
	  Local $sCisloSvazku = ControlGetText("Nástøihový plán vodièù","","Edit1")
	  Local $sCisloOP = ControlGetText("Nástøihový plán vodièù","","Edit15")
	  ...
	  Next
	  Local $sCisloOP = ControlGetFocus("Nástøihový plán vodièù","")
